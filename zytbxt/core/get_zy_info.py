...
url ='https://ks.hneao.cn/gaokao/v1/volunteer/yxjhk/zypage?pageSize=100&pageNo=1&zyzdm=0100004000106&yxdh=1113&lqpc=3&jhkl=2&jhlb=00&jhxz=0&tbrcdm=12&zylxdm=1&kskmyq=%E9%A6%96%E9%80%89%E7%89%A9%E7%90%86%EF%BC%8C%E5%86%8D%E9%80%89%E4%B8%8D%E9%99%90&_=1751160611579'
...
def update_url_page(url, zyzdm,yxdh,kskmyq):
    """更新URL中的page参数"""
    parsed = urlparse(url)
    query_params = parse_qs(parsed.query)
    query_params['yxdh'] = [str(yxdh)]
    query_params['zyzdm'] = [str(zyzdm)]
    query_params['kskmyq'] = [str(kskmyq)]
...
def get_query_mapping():
    """获取查询参数映射
    从temp_zyz_info目录下读取所有JSON文件，提取yxdh和zyzdm,kskmyq字段
    返回list[dict]：[{'yxdh': yxdh, 'zyzdm': zyzdm, 'kskmyq': kskmyq}]
    """
    result = []
    current_dir = get_current_dir_path()
    school_info_dir = current_dir.parent/'temp_zyz_info'
    ...
    print(f"共读取到 {len(result)} 条专业组的信息")
    return result
...
def crawl_data():
    ...
    for temp_dict in query_mapping:
        try:
            # 更新URL中的page参数
            current_url = update_url_page(url, temp_dict['zyzdm'],temp_dict['yxdh'],temp_dict['kskmyq'])
            
            print(f"正在爬取{temp_dict['yxdh']}的{temp_dict['zyzdm']}专业组...")
            ...
            # 保存为JSON文件
            filename = f"{temp_dict['yxdh']}-{temp_dict['zyzdm']}.json"
            current_dir = get_current_dir_path()
            file_path = current_dir.parent/'temp_zy_info'
            ...
            success_count += 1
            count += 1
            print(f"{temp_dict['yxdh']}院校代码,专业组{temp_dict['zyzdm']}数据已保存到 {full_file_name}")
            ...
            print(f"请求{temp_dict['yxdh']}-{temp_dict['zyzdm']}时发生错误: {e}")
            fail_school.append(temp_dict)
            continue
        except json.JSONDecodeError as e:
            print(f"解析{temp_dict['yxdh']}-{temp_dict['zyzdm']}JSON数据时发生错误: {e}")
            fail_school.append(temp_dict)
            continue
        except Exception as e:
            print(f"处理{temp_dict['yxdh']}-{temp_dict['zyzdm']}时发生未知错误: {e}")
            fail_school.append(temp_dict)   
            continue
