...
url ='https://ks.hneao.cn/gaokao/v1/zymk/lnfsxzyz/queryLnsjYxPage?yxdm=10003&_=1751175247743'
header_info = '''Accept: */*
...
Host: ks.hneao.cn
Referer: https://ks.hneao.cn/student/volunteer/volunteerSystem/lnsjList?yxdm=10003&pcdm=3
...
def update_url_page(url, yxdm):
    """更新URL中的page参数"""
    parsed = urlparse(url)
    query_params = parse_qs(parsed.query)
    query_params['yxdm'] = [str(yxdm)]
            ...
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                if 'result' in data and 'records' in data['result']:
                    for school in data['result']['records']:
                        if 'yxdm' in school and 'yxmc' in school:
                            yxdh_mapping[school['yxdm']] = school['yxmc']
...
def crawl_data():
    ...
    success_count = 0
    fail_school = []
    for yxdm, yxmc in yxdh_mapping.items():
        try:
            # 保存为JSON文件
            filename = f"{yxmc}.json"
            current_dir = get_current_dir_path()
            file_path = current_dir.parent/'temp_ls_zyz_info'
            file_path.mkdir(parents=True, exist_ok=True)
            full_file_name = file_path/filename
            if full_file_name.exists():
                print(f"{yxmc}数据已存在,跳过")
                count += 1
                success_count += 1
                continue
            # 更新URL中的page参数
            current_url = update_url_page(url, yxdm)
            ...
            success_count += 1
            count += 1
            print(f"{yxmc}名称,代号{yxdm}数据已保存到 {full_file_name}")
